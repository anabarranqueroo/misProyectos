...
export class EmpresaDetails implements OnInit {
  ...
  resenaRespondiendo: number | null = null;
  ...
  enviarResena(): void {
    ...
    this.resenaServicio.crear(resena).subscribe({
      next: () => {
        ...
        this.formResena.reset();
        this.estrellasSeleccionadas = 0;
        this.exitoResena = 'Tu reseña ha sido enviada correctamente y está pendiente de aprobación por el administrador. Puedes consultarla en tu perfil.';
        const id = Number(this.route.snapshot.paramMap.get('id'));
        this.cargarResenas(id);
        this.cargarMedia(id);
      },
  ...
  responderResena(id: number): void {
    this.resenaRespondiendo = id;
    this.textoRespuesta = '';
  }

  cancelarRespuesta(): void {
    this.resenaRespondiendo = null;
    this.textoRespuesta = '';
  }
  ...
  enviarRespuesta(): void {
    ...
    this.resenaServicio.responder(this.resenaRespondiendo!, this.textoRespuesta).subscribe({
      next: () => {
