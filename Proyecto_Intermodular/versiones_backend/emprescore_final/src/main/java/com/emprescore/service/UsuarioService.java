...
public class UsuarioService {
    ...
    public void resetPassword(String email, String passwordNueva) {
    ...
    public void delete(Long id) {
        ...
        // 4. Si tiene empresa, borrar reseñas y reportes de esa empresa primero
        empresaRepository.findByUsuarioId(id).ifPresent(empresa -> {
            resenaRepository.findByEmpresaId(empresa.getId()).forEach(resena -> {
                reporteRepository.findByResenaId(resena.getId()).forEach(r -> reporteRepository.deleteById(r.getId()));
